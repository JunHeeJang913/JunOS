...
typedef struct tag_gdtr {
  uint16_t m_limit;  // gdt의 크기
  uint32_t m_base;   // GDT의 시작 주소
} gdtr;
...
void get_set_descriptor(uint32_t i, uint64_t base, uint64_t limit,
...
int GDTInitialize() {
  ...
  // _gdtr 구조체의 주소는 페이징 전단계이며 실제 물리주소에 해당 변수가
  // 할당되어 있다. 디스크립터의 수를 나타내는 MAX_DESCRIPTORS의 값은 5이다.
  // NULL 디스크립터, 커널 코드 디스크립터, 커널 데이터 디스크립터, 유저 코드
  // 디스크립터 유저 데이터 디스크립터 이렇게 총 5개이다. 디스크립터당
  // 6바이트이므로 GDT의 크기는 30바이트다.
